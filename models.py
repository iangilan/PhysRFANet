...
'''
class RFAAttUNet(nn.Module): # works well but worse than RFACNN
    def __init__(self, in_channels=2, out_channels=1, in_inter=True, out_inter=True):
        super(RFAAttUNet, self).__init__()
    ...
    def forward(self, Ninput, MR):
        ...
        batch_size, C, D, H, W = m2.size()  # Get the size for reshaping later
        m2_flat = m2.view(batch_size, C, -1).permute(0, 2, 1)  # Reshape to (batch_size, num_patches, embed_dim) and permute for MHA
        m2_att, _ = self.mha(m2_flat, m2_flat, m2_flat)
        m2 = m2_att.permute(0, 2, 1).reshape(batch_size, C, D, H, W)  # Reshape back to original shape and permute
...
class RFAAttUNet(nn.Module):
    def __init__(self, in_channels=2, out_channels=1, in_inter=True, out_inter=True):
        ...
        self.down1 = self.unet_down(in_channels, 64, normalize=True)
        self.down2 = self.unet_down(64, 128)
        self.down3 = self.unet_down(128, 256)
    ...
    def self_attention(channels):
        ...
        return mha, ln, ff_self, channels
   
    def apply_attention(self, attention_layers, x):
        mha, ln, ff_self, channels = attention_layers
        device = x.device
        mha = mha.to(device)
        ln = ln.to(device)
        ff_self = ff_self.to(device)
        size = x.shape[-1]
        x = x.view(-1, channels, size * size * size).swapaxes(1, 2)
        x_ln = ln(x)
        attention_value, _ = mha(x_ln, x_ln, x_ln)
        attention_value = attention_value + x
        attention_value = ff_self(attention_value) + attention_value
        return attention_value.swapaxes(2, 1).contiguous().view(-1, channels, size, size, size)
